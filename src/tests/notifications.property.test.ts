// TODO(it-ticket-management/6.6): NotificationService now persists through
// NotificationRepository instead of an in-memory Map. The static helpers
// (clearAll/addToStore/getStore) and the synchronous getUserNotificationsList
// have been removed. These tests need to be rewritten using a
// NotificationRepository mock and async getUserNotificationsList; see spec
// task 6.6.
...
describe("Property 12: Notification Ordering", () => {
  ...
  it("notifications are returned ordered by createdAt DESC (newest first)", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(fc.tuple(timestampArb, notificationTypeArb), {
          minLength: 2,
          maxLength: 20,
        }),
  ...
  it("marking a non-existent notification returns an error", async () => {
    await fc.assert(
      fc.asyncProperty(fc.uuid(), async (fakeNotificationId) => {
        NotificationService.clearAll();
  ...
  it("marking another user's notification returns an error", async () => {
    await fc.assert(
      fc.asyncProperty(notificationTypeArb, async (type) => {
        NotificationService.clearAll();
