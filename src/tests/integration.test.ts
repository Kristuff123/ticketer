// TODO(it-ticket-management/6.6): NotificationService is now repository-backed
// (constructor requires NotificationRepository, no static clearAll/getStore).
// These integration tests need to be rewritten against the repository layer
// (or a NotificationRepository test double); see spec task 6.6. Several other
// constructor-signature failures in this file come from earlier tasks (6.1,
// 6.3, 6.5) and are tracked there.
...
function createTicketInStore(
  ...
  const ticket: Ticket = {
    ...
    description:
      overrides.description ?? "Test description for integration testing",
...
describe("Integration: Full Ticket Lifecycle", () => {
  ...
  describe("Create → Assign → Comment → Resolve → Close", () => {
    it("should complete the full ticket lifecycle with correct state at each step", async () => {
      ...
      expect(assignResult.ticket.history[0].action).toBe(
      ...
      const commentResult = await ticketService.addComment(
        ticket.id,
        {
          ticketId: ticket.id,
          authorId: "tech-001",
          content: "Paper jam cleared, testing now.",
          isInternal: false,
        },
        "tech-001",
      );
      ...
      expect(commentResult.comment.content).toBe(
        "Paper jam cleared, testing now.",
      );
      ...
      expect(resolveResult.ticket.history[1].action).toBe(
        HistoryActionType.STATUS_CHANGED,
      );
      expect(resolveResult.ticket.history[1].previousValue).toBe(
        TicketStatus.IN_PROGRESS,
      );
      expect(resolveResult.ticket.history[1].newValue).toBe(
        TicketStatus.RESOLVED,
      );
      ...
      expect(closeResult.ticket.history[2].action).toBe(
        HistoryActionType.STATUS_CHANGED,
      );
      expect(closeResult.ticket.history[2].previousValue).toBe(
        TicketStatus.RESOLVED,
      );
  ...
  describe("Permissions: RBAC Enforcement", () => {
    ...
    it("reporter can only view and close own tickets", async () => {
      ...
      // Can view own ticket
      expect(
        await ticketLookupService.hasPermission(
          "reporter-001",
          "view",
          ticket.id,
        ),
